...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("Dive") and dash:
		print(abs($".".rotation_degrees.y) * 0.3)
		velocity.x = velocity.x + (direction.x * DASHSPEED)
		velocity.y = velocity.y + ((abs($".".rotation_degrees.y) * 0.3) + DASHSPEED)
		velocity.z = velocity.z + (direction.z * DASHSPEED) 
		dash = false
		print("dash used")
	else:
		if direction:
